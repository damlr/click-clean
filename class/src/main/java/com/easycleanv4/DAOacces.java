...
public class DAOacces {
	...
	public void addCleaner(Cleaner a) {
		try {
			String strQuery = "INSERT INTO user (firstName,secondName,userName,email,passWord,age,phoneNumber,dateOfBirth,statut) VALUES('"
					+ a.getFirstName() + "','" + a.getSecondName() + "','" + a.getUsername() + "','" + a.getEmail()
					+ "','" + a.getPassword() + "'," + a.getAge() + "," + a.getPhoneNumber() + ",'" + a.getDateOfBirth()
					+ "','" + a.getStatut() + "');";
	...
	public boolean connection(String username, String mdp) {
		try {
			String strQuery = "SELECT userName,passWord FROM user WHERE userName = '" + username + "' AND passWord = '"
					+ mdp + "';";
